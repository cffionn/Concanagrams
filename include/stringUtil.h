...
inline std::string getTimeStr()
{
  std::time_t now = time(0);
  std::tm *ltm = std::localtime(&now);
  std::string timeStr = std::to_string(ltm->tm_min);
  if(timeStr.size() == 1) timeStr = "0" + timeStr;
  std::string secStr = std::to_string(ltm->tm_sec);
  if(secStr.size() == 1) secStr = "0" + secStr;

  std::string hourStr = std::to_string(ltm->tm_hour);
  if(hourStr.size() == 1) hourStr = "0" + hourStr;

  timeStr = hourStr + timeStr + secStr;
